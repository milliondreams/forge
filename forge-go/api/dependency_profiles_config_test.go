...
func TestNomicProfileIsReturnedOnlyForEmbeddings(t *testing.T) {
	...
	require.Contains(t, dependencyEntryKeys(embeddingProfiles), "embeddings", "credential readiness must not hide profiles")
}

func TestCredentialPresenceDoesNotAffectProfileAvailability(t *testing.T) {
	configPath := filepath.Join("..", "conf", "agent-dependencies.yaml")
	t.Setenv("OPENAI_API_KEY", "ignored")

	ready, err := ReadyDependencyProfileKeys(configPath, func(string) (bool, error) { return false, nil })
	require.NoError(t, err)
	require.Contains(t, ready, "embeddings")
}
